...
>
> **实施进度（2026-08）**：S1 工程基建 + S2 承诺闭环 + S3 更多部位（B-1..B-6 含服装层/非标准部位/adult 分级）+ S4 Demo 矩阵（A1–A6 全部）+ S5 加固与性能（R-P2-1 fuzz / R-P2-2 审核链 / R-P2-3 基准 / O-3 lint / O-4 词典）+ 优化项 O-1 时钟 / O-5 verify / O-6 文档 + **Phase-2 真实几何接线（toL2dmArtifact moc3Bytes）** + **S6 部分交付 v1.18（P6 驱动项：library 索引/词表 manifest 生成器/scene 舞台/TTS 升级/MCP 表层 + P4 完整 LLM 创作通道：LlmDesigner/LlmRepairer）** 已全部完成（changelog v1.0–v1.18），实测 254 测试全绿 + typecheck 9 包 + eval 6/6+3/3 + lint 全一致。剩余：S6 之 parts-naming 扩展 / PSD 导出回环 / 场景与装配台 UI（属宿主或按路线图续启）。
>
...
| v1.18 | 2026-08（S6 部分交付：驱动 P6 + 创作 P4 完整通道） | ① 驱动 P6：library 索引生成器（driver catalog.ts `buildBehaviorIndex` + twohop `BehaviorIndex` 同优先级 `weight` 种子加权随机 + `pickWeighted`，§14.3-2）、词表 manifest 生成器（driver manifest.ts `generateManifest`/`vocabularyOf`/`generateLibraryIndex`）、MCP 表层（driver mcp.ts `driverToolCatalog`，E6）、TTS 升级（driver tts/phonemes.ts `phonemeToViseme` 音素→口型 + tts/viseme.ts `blendVisemes` 60–80ms 混合 + estimate.ts 音节视素+语调韵律）；② 创作 P4：create `Designer/DesignContext` 注入点（loop.ts，Repairer 契约放宽为可异步）+ host `LlmDesigner`（few-shot 生成整条 CreationDirective）/`LlmRepairer`（校验问题回注 → LLM 修正）/`llm-directive.ts`（`sanitizeCreationDirective` 确定性清洗 + 主色提取 + 设计/修复提示词）；③ engine `SceneStage` 场景舞台（多角色+相机 pan/zoom+背景合成，`L2dmPlayer.renderFrame` 抽取）；实测 254 测试全绿 + typecheck 9 包 + eval 6/6+3/3 + lint 全一致 |
